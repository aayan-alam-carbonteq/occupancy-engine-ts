...
export function master_adjudication_user_prompt(
  ...
  return [
    ...
    "- records_read.occupancy_signal — pick by WHAT THE ROWS CONTAIN, not by how sure you feel:",
    "  non_owner_occupancy — the records point to someone other than the owner occupying the subject,",
    "  and nothing substantive points the other way.",
    "  owner_occupancy — the records point to the owner occupying the subject, and nothing substantive",
    "  points the other way.",
    "  conflicting — the records contain SUBSTANTIVE evidence BOTH ways and you cannot order it in",
    "  time. Owner mailing at the subject alongside unrelated occupants in utility or address-history",
    "  rows is the canonical case. This is a POSITIVE finding about a well-populated address, not a",
    "  failure to decide: choose it whenever both readings have real rows behind them, however little",
    "  you can say about which is current.",
    "  no_signal — there is nothing to read: rows are absent, or so thin or purely administrative that",
    "  neither reading has evidence. no_signal is about ABSENCE OF ROWS ONLY. If the address has",
    "  substantive rows on both sides, that is conflicting, not no_signal. It is never a weak",
    "  owner_occupancy, and never a way to express uncertainty about a signal you did see.",
    "- records_read.strength is how much weight the records carry for that signal.",
    "  It is not how confident you feel, and NOT how current or well-dated the rows are.",
    "  weak: one thin or low-reliability row. moderate: a clear reading from a single source family.",
    "  strong: the same reading reappears across independent source families (for example utility and",
    "  trace and tax, or address-history and loan).",
    "  Undated or stale rows do NOT cap strength — breadth of corroboration sets strength, and",
    "  recency is reported separately in clarity_score. Many rows agreeing across families is strong",
    "  even when not one of them carries a date. For conflicting, strength is the weight of the",
    "  WEAKER side, since that is what keeps the case unresolved.",
