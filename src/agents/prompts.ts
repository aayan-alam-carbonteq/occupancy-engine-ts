...
const SAME_PERSON_REQUIREMENT_LINES: readonly string[] = [
  ...
  "  One human per group, and each P id in one group only (a household row joins only one of its people).",
...
export function master_adjudication_user_prompt(
  ...
  return [
    ...
    "- records_read.nonowner_occupancy_strength is 0-10: how strongly the records point AWAY from",
    "  owner occupancy. 0 = the records clearly place the owner in residence; 5 = genuinely balanced;",
    "  10 = they clearly place someone else there. It must reflect case-level interpretation, not the",
    "  raw analyst sum. Set it from the records only — you have been shown no outside claim.",
