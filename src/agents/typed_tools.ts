// The "typed_tools" retrieval surface: a fixed set of tools (get_records, get_people, search_people,
// and per-shape get_* tools), the dispatch logic (run_typed_tool), and the per-heuristic tool guide
// text.
//
// Each tool is defined with LangChain's tool(func, { name, description, schema }); the schema is a
// zod object. The func body is a stub (async () => ({})) because the subagent loop routes by tool
...
function _envelope(source: string, block: Record<string, any>): Record<string, any> {
  return {
    ok: true,
    source,
    count: Math.trunc(Number(block["totalCount"] ?? 0)),
...
function _normalize_shapes(args: Record<string, any>, agent_input: HeuristicAgentInput): string[] {
  ...
  if (shapes.length === 0) {
    ...
    const scope = (
      heuristic["context_scope"]?.length
        ? heuristic["context_scope"]
        : heuristic["input_sources"]?.length
          ? heuristic["input_sources"]
          : []
    ) as any[];
    const scopeShapes = scope.filter((s) => String(s).trim() !== "").map((s) => String(s).trim().toLowerCase());
    shapes = scopeShapes.length > 0 ? scopeShapes : ["tax", "base"];
  }
...
export function typed_tools_guide(heuristic: Record<string, any>): string {
  ...
  const scope = rawScope.filter((s) => String(s).trim() !== "").map((s) => String(s).trim().toLowerCase());
  const suggestedShapes = scope.filter((s) => ALL_SHAPES.has(s));
  const suggested = suggestedShapes.length > 0 ? suggestedShapes : ["tax", "base"];
  ...
  const lines = [
    ...
    "- get_records(shapes=[...], person_id?=...): pass multiple shapes at once " +
      ...
      `shapes=${JSON.stringify(suggested)}.`,
    "Entity discovery:",
    "- get_people: people at the subject address (returns person ids)",
    "- search_people: find a person entity by name (e.g. a tax owner who may live elsewhere)",
    "Drill-down only (use get_records first; these fetch ONE shape, for pagination or a single follow-up):",
  ];
  const drill = relevant_tools.length > 0 ? relevant_tools : ["get_tax", "get_base"];
